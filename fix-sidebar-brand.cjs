...
const regexBrand = /<div className="sidebar-brand">[\s\S]*?<button className="mobile-close-btn" onClick=\{.*\}>[\s\S]*?<\/button>\s*<\/div>/;
const matchBrand = code.match(regexBrand);
...
if (matchBrand) {
  ...
  code = code.replace(matchBrand[0], newBrand);
  console.log('Replaced brand');
}

const regexUserInfo = /<div className="user-info">\s*<span className="user-name">\{user\?\.name\?\.split\(' '\)\[0\] \|\| 'Usuário'\}<\/span>\s*<span className="user-role" style=\{\{ color: roleColor \}\}>\{roleLabel\.toUpperCase\(\)\}<\/span>\s*<\/div>/;
const matchUserInfo = code.match(regexUserInfo);
...
if (matchUserInfo) {
  ...
  code = code.replace(matchUserInfo[0], newUserInfo);
  console.log('Replaced user info');
}
