...
const AgendaCS = ({ clients }) => {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    const { data: meetingsData } = await supabase.from('client_meetings').select('*');
    ...
    try {
      ...
      if (newEvent.category === 'Agendamento') {
        ...
        const { error } = await supabase.from('department_tasks').insert([{
          ...
          department: 'Sucesso do Cliente',
          status: 'a_fazer',
          metadata: { deadline: startIso, end_time: endIso, notes: newEvent.notes, guests: newEvent.guests, assigned_by: 'Sucesso do Cliente' },
          assigned_to: currentUser?.name || 'Sucesso do Cliente'
        }]);
        if (error) throw error;
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16, height:'100%', minHeight:0 }}>
      ...
      <div className="cal-layout-responsive" style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:16, flex:1, minHeight:0 }}>
        ...
        <div className="cal-panel-responsive" style={{ display:'flex', flexDirection:'column', gap:14, minHeight:0, overflow:'hidden' }}>
          ...
          {/* Próximas Reuniões */}
          <div className="glass-panel" style={{ padding:14, borderRadius:12, flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
